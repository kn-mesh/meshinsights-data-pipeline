# uv run python -m src.plugins.adx_plugin
...
class ADXConnectionManager:
    ...
    def get_connection(self):
        ...
        if not self._connection_established:
            self.KUSTO_DATABASE, self.KUSTO_CLIENT = self._establish_adx_connection()
            self._connection_established = True
        return self.KUSTO_DATABASE, self.KUSTO_CLIENT

    def _establish_adx_connection(self):
        """
        ...
        AAD_TENANT_ID = os.environ.get('AZURE_TENANT_ID')
...
class ADXConnector(DBConnector):
    ...
    def connect(self) -> Tuple[str, KustoClient]:
        ...
        """
        try:
            self.database, self.client = ADXConnectionManager().get_connection()
    ...
    def execute_query(self, query: str) -> Any:
        """
        Execute a query against ADX with retry logic.
        
        Args:
            query (str): A Kusto query string
            
        Returns:
            KustoResponseDataSet: The response from the ADX client
        ...
        while attempts < self.max_retries:
            try:
                if not self.is_connected:
                    self.connect()
                #print(f"Executing query (attempt {attempts + 1}/{self.max_retries})...")
    ...
    def disconnect(self) -> None:
        ...
        """
        if self.client:
            # ADX client might not have explicit close method
...
| project TableName
"""
    response = adx_plugin.execute_query(query)

    # Assume response.primary_results[0] is a Kusto table; convert it to a DataFrame
